...
class Graph:  # graful problemei
   ...
   def succesori(self, nodCurent):
      ...
      for istiva, stiva in enumerate(nodCurent.info):
         ...
         for istiva2, stiva2 in enumerate(copieStiva):
            ...
            stareNou = copy.deepcopy(copieStiva)
            stareNou[istiva2].append(bloc)

            nodNou = NodParcurgere(stareNou, nodCurent.g + 1, self.estimeaza_h(stareNou), nodCurent)
   ...
   def valideaza(self):
      ...
      cond2 = all([multimeStart == set(sum(scop, start=[])) for scop in self.scopuri])
